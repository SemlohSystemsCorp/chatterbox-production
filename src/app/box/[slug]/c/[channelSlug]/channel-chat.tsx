...
import { Hash, Lock, Users, SendHorizontal, Pencil, Trash2, X, Check, Smile, MessageSquare, Paperclip, FileText, UserPlus } from "lucide-react";
...
export function ChannelChat({
  ...
  const openEmojiPicker = useCallback((msgId: string, button: HTMLButtonElement) => {
    ...
    // If it would go below the viewport, position above the button
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .subscribe((status: string, err?: Error) => {
        console.log(`[Realtime] messages:${channel.id}:`, status, err ?? "");
      });
  ...
  useEffect(() => {
    ...
    presenceChannel
      .on("presence", { event: "sync" }, () => {
        ...
        const typing = Object.keys(state).filter((id) => id !== currentUserId);
        setTypingUsers(typing);
      })
      .subscribe((status: string, err?: Error) => {
        console.log(`[Realtime] typing:${channel.id}:`, status, err ?? "");
      });
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    if (msg) {
      ...
      for (const file of filesToUpload) {
        ...
        const path = `${channel.id}/${msg.id}/${crypto.randomUUID()}.${ext}`;
      ...
      setMessages((prev) => {
        ...
        if (realtimeAlreadyAdded) {
          // Realtime beat us — remove optimistic, update realtime entry with attachments
          return prev
  ...
  function renderContent(text: string) {
    // Split on @username patterns and render them as bold spans
